...
import { range as rangeArr, getFirstDayOfMonth, getPrevMonthLastDays, getMonthDays, getI18nSettings, validateRangeInOneMonth } from 'boss-element-ui/src/utils/date-util';
...
export default {
  ...
  computed: {
    ...
    formatedToday() {
      return this.bossCalendar.formatedToday;
    },
        ...
        <tbody>
          {
            this.rows.map((row, index) => <tr
              class={{
                'boss-calendar-table__row': true,
                'boss-calendar-table__row--hide-border': index === 0 && this.hideHeader
              }}
              key={index}>
              {
