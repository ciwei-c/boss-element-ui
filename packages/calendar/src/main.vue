<template>
  <div class="boss-calendar">
    <div class="boss-calendar__header">
      <div class="boss-calendar__title">
        {{ i18nDate }}
      </div>
      <div
        class="boss-calendar__button-group"
        ...
        <boss-button-group>
          <boss-button
            ...
            {{ t('boss.datepicker.prevMonth') }}
          </boss-button>
          <boss-button
            type="plain"
            size="mini"
            @click="selectDate('today')">
            {{ t('boss.datepicker.today') }}
          </boss-button>
          <boss-button
            type="plain"
            size="mini"
            @click="selectDate('next-month')">
            {{ t('boss.datepicker.nextMonth') }}
          </boss-button>
        </boss-button-group>
      </div>
    </div>
    <div
      class="boss-calendar__body"
      v-if="validatedRange.length === 0"
      key="no-range">
      <date-table
        :date="date"
...
import { validateRangeInOneMonth } from 'boss-element-ui/src/utils/date-util';
...
export default {
  ...
  computed: {
    ...
    i18nDate() {
      ...
      return `${year} ${this.t('boss.datepicker.year')} ${this.t('boss.datepicker.month' + month)}`;
    },
