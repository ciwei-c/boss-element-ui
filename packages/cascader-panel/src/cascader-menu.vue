...
export default {
  ...
  methods: {
    ...
    renderEmptyText(h) {
      return (
        <div class="boss-cascader-menu__empty-text">{ this.t('boss.cascader.noData') }</div>
      );
    },
    renderNodeList(h) {
      const { menuId } = this;
      ...
      return [
        ...nodes,
        isHoverMenu ? <svg ref='hoverZone' class='boss-cascader-menu__hover-zone'></svg> : null
      ];
    }
  },
  ...
  render(h) {
    ...
    return (
      <boss-scrollbar
        ...
        role="menu"
        id={ menuId }
        class="boss-cascader-menu"
        wrap-class="boss-cascader-menu__wrap"
        view-class={{
          'boss-cascader-menu__list': true,
          'is-empty': isEmpty
        }}
        { ...events }>
