<template>
  <div
    ...
    :class="[
      'boss-cascader',
      realSize && `boss-cascader--${realSize}`,
    ...
    <boss-input
      ...
      <template slot="suffix">
        <i
          ...
          key="clear"
          class="boss-input__icon boss-icon-circle-close"
    ...
    <div v-if="multiple" class="boss-cascader__tags">
      <boss-tag
      ...
      <input
        ...
        type="text"
        class="boss-cascader__search-input"
    ...
    <transition name="boss-zoom-in-top" @after-leave="handleDropdownLeave">
      <div
        v-show="dropDownVisible"
        ref="popper"
        :class="['boss-popper', 'boss-cascader__dropdown', popperClass]">
        <boss-cascader-panel
          ref="panel"
          v-show="!filtering"
          ...
          @close="toggleDropDownVisible(false)"></boss-cascader-panel>
        <boss-scrollbar
          ref="suggestionPanel"
          v-if="filterable"
          v-show="filtering"
          tag="ul"
          class="boss-cascader__suggestion-panel"
          view-class="boss-cascader__suggestion-list"
          ...
          <template v-if="suggestions.length">
            <li
              ...
              :key="item.uid"
              :class="[
                'boss-cascader__suggestion-item',
                item.checked && 'is-checked'
              ]"
              :tabindex="-1"
              @click="handleSuggestionClick(index)">
              <span>{{ item.text }}</span>
              <i v-if="item.checked" class="boss-icon-check"></i>
            </li>
          </template>
...
import BossCascaderPanel from 'boss-element-ui/packages/cascader-panel';
import AriaUtils from 'boss-element-ui/src/utils/aria-utils';
import { t } from 'boss-element-ui/src/locale';
import { isEqual, isEmpty, kebabCase } from 'boss-element-ui/src/utils/util';
import { isUndefined, isFunction } from 'boss-element-ui/src/utils/types';
import { isDef } from 'boss-element-ui/src/utils/shared';
...
export default {
  ...
  computed: {
    realSize() {
      const _bossFormItemSize = (this.bossFormItem || {}).bossFormItemSize;
      return this.size || _bossFormItemSize || (this.$ELEMENT || {}).size;
    },
    tagSize() {
  ...
  methods: {
    ...
    focusFirstNode() {
      this.$nextTick(() => {
        ...
        if (filtering && suggestionPanel) {
          firstNode = suggestionPanel.$el.querySelector('.boss-cascader__suggestion-item');
        } else {
          const firstMenu = popper.querySelector('.boss-cascader-menu');
          firstNode = firstMenu.querySelector('.boss-cascader-node[tabindex="-1"]');
        }
    ...
    updateStyle() {
      ...
      const inputInner = $el.querySelector('.boss-input__inner');

      if (!inputInner) return;

      const tags = $el.querySelector('.boss-cascader__tags');
      let suggestionPanelEl = null;

      if (suggestionPanel && (suggestionPanelEl = suggestionPanel.$el)) {
        const suggestionList = suggestionPanelEl.querySelector('.boss-cascader__suggestion-list');
