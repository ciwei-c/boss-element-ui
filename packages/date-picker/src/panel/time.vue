<template>
  <transition name="boss-zoom-in-top" @after-leave="$emit('dodestroy')">
    <div
      v-show="visible"
      class="boss-time-panel boss-popper"
      :class="popperClass">
      <div class="boss-time-panel__content" :class="{ 'has-seconds': showSeconds }">
        <time-spinner
          ref="spinner"
          @change="handleChange"
      ...
      <div class="boss-time-panel__footer">
        ...
        <button
          type="button"
          class="boss-time-panel__btn"
          :class="{confirm: !disabled}"
          @click="handleConfirm()">{{ t('boss.datepicker.confirm') }}</button>
      </div>
    </div>
  </transition>
</template>
