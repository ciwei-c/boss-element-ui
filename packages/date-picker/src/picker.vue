<template>
  <div class="boss-date-editor-wrap">
    <boss-input
      ...
      ref="reference"
    >
      <i slot="prefix" class="boss-input__icon" :class="triggerClass" @click="handleFocus"></i>
      <i
        slot="suffix"
    ...
    <div
      class="boss-date-editor boss-range-editor boss-input__inner"
      :class="[
      'boss-date-editor--' + type,
      pickerSize ? `boss-range-editor--${ pickerSize }` : '',
      ...
      v-else
    >
      <i :class="['boss-input__icon', 'boss-range__icon', triggerClass]"></i>
      <input
        autocomplete="off"
        ...
        @focus="handleFocus"
        class="boss-range-input"
      />
      <slot name="range-separator">
        <span class="boss-range-separator">{{ rangeSeparator }}</span>
      </slot>
      <input
        autocomplete="off"
...
export default {
  ...
  props: {
    ...
    clearIcon: {
      type: String,
      default: "boss-icon-circle-close"
    },
    name: {
      default: "",
      validator
  ...
  components: { BossInput, PickerPanel },
    ...
    pickerVisible(val) {
      ...
      if (val) {
        ...
        if (this.validateEvent) {
          this.dispatch("BossFormItem", "boss.form.blur");
        }
        this.$emit("blur", this);
        this.blur();
      }
  ...
  computed: {
    ...
    triggerClass() {
      return (
        this.prefixIcon ||
        (this.type.indexOf("time") !== -1 ? "boss-icon-time" : "boss-icon-date")
      );
    },
  ...
  methods: {
    ...
    handleStartChange(event) {
      ...
      if (value) {
        ...
        const newValue = [value, this.picker.dateValue && this.picker.dateValue[1]];
        this.picker.dateValue = newValue;
    ...
    handleEndChange(event) {
      ...
      if (value) {
        ...
        const newValue = [this.picker.dateValue && this.picker.dateValue[0], value];
        this.picker.dateValue = newValue;
        if (this.isValidValue(newValue)) {
          this.emitInput(newValue);
          this.userInput = null;
    ...
    showPicker() {
      ...
      this.picker.dateValue = this.parsedValue;
      this.picker.resetView && this.picker.resetView();
    ...
    mountPicker() {
      ...
      this.picker = this.$refs.panel.getPanel();
