<script>
  ...
  import BossButtonGroup from 'boss-element-ui/packages/button-group';
  import { generateId } from 'boss-element-ui/src/utils/util';

  export default {
    name: 'BossDropdown',

    componentName: 'BossDropdown',
    ...
    watch: {
      visible(val) {
        this.broadcast('BossDropdownMenu', 'visible', val);
        this.$emit('visible-change', val);
      },
      focusing(val) {
        const selfDefine = this.$el.querySelector('.boss-dropdown-selfdefine');
        if (selfDefine) { // 自定义
          if (val) {
    ...
    methods: {
      ...
      initAria() {
        ...
        if (!this.splitButton) { // 自定义
          ...
          this.triggerElm.setAttribute('class', (this.triggerElm.getAttribute('class') || '') + ' boss-dropdown-selfdefine'); // 控制
        }
      },
      initEvent() {
    ...
    render(h) {
      ...
      let triggerElm = !splitButton
        ? this.$slots.default
        : (<boss-button-group>
          <boss-button type={type} size={dropdownSize} nativeOn-click={handleMainButtonClick}>
            {this.$slots.default}
          </boss-button>
          <boss-button ref="trigger" type={type} size={dropdownSize} class="boss-dropdown__caret-button">
            <i class="boss-dropdown__icon boss-icon-arrow-down"></i>
          </boss-button>
        </boss-button-group>);

      return (
        <div class="boss-dropdown" v-clickoutside={hide}>
          {triggerElm}
          {this.$slots.dropdown}
        </div>
      );
