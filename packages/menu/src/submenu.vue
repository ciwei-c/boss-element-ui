<script>
  ...
  export default {
    ...
    computed: {
      ...
      menuTransitionName() {
        return this.rootMenu.collapse ? 'boss-zoom-in-left' : 'boss-zoom-in-top';
      },
      opened() {
      ...
      isFirstLevel() {
        ...
        while (parent && parent !== this.rootMenu) {
          if (['BossSubmenu', 'BossMenuItemGroup'].indexOf(parent.$options.componentName) > -1) {
            isFirstLevel = false;
            break;
          } else {
    ...
    methods: {
      ...
      handleClick() {
        ...
        this.dispatch('BossMenu', 'submenu-click', this);
      },
      handleMouseenter(event, showTimeout = this.showTimeout) {
        ...
        this.dispatch('BossSubmenu', 'mouse-enter-child');
      ...
      handleMouseleave(deepDispatch = false) {
        ...
        this.dispatch('BossSubmenu', 'mouse-leave-child');
        clearTimeout(this.timeout);
        this.timeout = setTimeout(() => {
        ...
        if (this.appendToBody && deepDispatch) {
          if (this.$parent.$options.name === 'BossSubmenu') {
    ...
    render(h) {
      ...
      const popupMenu = (
        <transition name={menuTransitionName}>
          <div
            ref="menu"
            v-show={opened}
            class={[`boss-menu--${mode}`, popperClass]}
            ...
            <ul
              role="menu"
              class={['boss-menu boss-menu--popup', `boss-menu--popup-${currentPlacement}`]}
      ...
      const inlineMenu = (
        <boss-collapse-transition>
          <ul
            role="menu"
            class="boss-menu boss-menu--inline"
            v-show={opened}
            style={{ backgroundColor: rootMenu.backgroundColor || '' }}>
            {$slots.default}
          </ul>
      ...
      ) ? 'boss-icon-arrow-down' : 'boss-icon-arrow-right';
      ...
      return (
        ...
        >
          ...
          >
            {$slots.title}
            <i class={[ 'boss-submenu__icon-arrow', submenuTitleIcon ]}></i>
          </div>
