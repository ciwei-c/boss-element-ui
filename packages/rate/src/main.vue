  ...
  <div
    ...
    <span
      ...
      <i
        ...
        class="boss-rate__icon"
        :style="getIconStyle(item)">
        <i
          v-if="showDecimalIcon(item)"
          :class="decimalIconClass"
          :style="decimalStyle"
          class="boss-rate__decimal">
        </i>
      </i>
    </span>
    <span v-if="showText || showScore" class="boss-rate__text" :style="{ color: textColor }">{{ text }}</span>
  </div>
</template>

<script>
  import { hasClass } from 'boss-element-ui/src/utils/dom';
  import { isObject } from 'boss-element-ui/src/utils/types';
  ...
  export default {
    ...
    props: {
      ...
      iconClasses: {
        type: [Array, Object],
        default() {
          return ['boss-icon-star-on', 'boss-icon-star-on', 'boss-icon-star-on'];
        }
      },
      voidIconClass: {
        type: String,
        default: 'boss-icon-star-off'
      },
      disabledVoidIconClass: {
        type: String,
        default: 'boss-icon-star-on'
      },
      disabled: {
        type: Boolean,
        default: false
    ...
    methods: {
      ...
      setCurrentValue(value, event) {
        ...
        if (this.allowHalf) {
          ...
          if (hasClass(target, 'boss-rate__item')) {
            target = target.querySelector('.boss-rate__icon');
          }
          if (hasClass(target, 'boss-rate__decimal')) {
