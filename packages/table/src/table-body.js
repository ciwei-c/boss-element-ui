...
export default {
  ...
  render(h) {
    this.editorBasic.clearEditorMap()
    const data = this.data || [];
    return (
      <table
        class="boss-table__body"
        cellspacing="0"
        cellpadding="0"
        border="0">
        <colgroup>
  ...
  watch: {
    'store.states.hoverRow'(newVal, oldVal) {
      ...
      raf(() => {
        const rows = this.$el.querySelectorAll('.boss-table__row');
  ...
  methods: {
    ...
    getRowClass(row, rowIndex) {
      const classes = ['boss-table__row'];
      if (this.table.highlightCurrentRow && row === this.store.states.currentRow) {
        classes.push('current-row');
      }

      if (this.stripe && rowIndex % 2 === 1) {
        classes.push('boss-table__row--striped');
      }
    ...
    handleCellMouseEnter(event, row) {
      ...
      if (!(hasClass(cellChild, 'boss-tooltip') && cellChild.childNodes.length)) {
        return;
      }
