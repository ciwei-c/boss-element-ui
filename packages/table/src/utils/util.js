...
export const getExpandData = function(source, targetData){
  if(!targetData) targetData = []
  source.forEach(v=>{
    if(v.children){
      getExpandData(v.children, targetData)
    }else{
      targetData.push(v)
    }
  })
...
export const getColumnByCell = function(table, cell) {
  const matches = (cell.className || '').match(/boss-table_[^\s]+/gm);
  if (matches) {
...
export function toggleRowStatus(statusArr, row, newVal, data) {
  let changed = false;
  const index = statusArr.indexOf(row);
  const included = index !== -1;

  const addRow = () => {
    if(data.indexOf(statusArr[statusArr.length - 1]) > data.indexOf(row)){
      statusArr.unshift(row);
    }else{
      statusArr.push(row);
    }
    changed = true;
