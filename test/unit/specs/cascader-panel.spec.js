...
const getValidOptions = (el, menuIndex) => getMenus(el)[menuIndex].querySelectorAll('.boss-cascader-node[tabindex="-1"]');
const getLabel = el => el.querySelector('.boss-cascader-node__label').textContent;
...
describe('CascaderPanel', () => {
    ...
    vm = createVue({
      template: `
        <boss-cascader-panel
          ref="panel"
          v-model="value"
          :options="options"></boss-cascader-panel>
      `,
      data() {
        return {
          value: [],
  ...
  it('with default value', async() => {
    ...
    expect(getOptions(el, 2)[0].querySelector('.boss-icon-check')).to.exist;
  });
  ...
  it('expand by hover', async() => {
    vm = createVue({
      template: `
        <boss-cascader-panel
          :options="options"
          :props="props"></boss-cascader-panel>
      `,
      data() {
        return {
          options,
  ...
  it('multiple mode', async() => {
    ...
    const el = vm.$el;
    const checkbox = getOptions(el, 0)[0].querySelector('.boss-checkbox');
    expect(checkbox).to.exist;
    expect(checkbox.querySelector('.boss-checkbox__input').className).to.not.includes('is-checked');
    checkbox.querySelector('input').click();

    await waitImmediate();
    expect(checkbox.querySelector('.boss-checkbox__input').className).to.includes('is-checked');
  ...
  it('multiple mode with disabled default value', async() => {
    ...
    await waitImmediate();
    expect(checkbox).to.exist;
    expect(checkbox.querySelector('.boss-checkbox__input').className).to.includes('is-indeterminate');
    checkbox.querySelector('input').click();
  ...
  it('check strictly in single mode', async() => {
    ...
    const el = vm.$el;
    const radio = getOptions(el, 0)[0].querySelector('.boss-radio');
    ...
    expect(getOptions(el, 0)[1].querySelector('.boss-radio').className).to.includes('is-disabled');
  });
  ...
  it('check strictly in multiple mode', async() => {
    ...
    getOptions(el, 0)[0].click();
    await waitImmediate();
    expect(getOptions(el, 1)[0].querySelector('.boss-checkbox').className).to.not.includes('is-checked');
    getOptions(el, 1)[0].querySelector('input').click();
    await waitImmediate();
    expect(vm.value).to.deep.equal(['zhejiang', 'hangzhou']);
    expect(getOptions(el, 0)[1].querySelector('.boss-checkbox').className).to.includes('is-disabled');
  });
  ...
  it('dynamic loading', async() => {
    ...
    firstOption.click();
    await waitImmediate();
    expect(firstOption.querySelector('i').className).to.includes('boss-icon-loading');
    await wait(1000);
    expect(firstOption.querySelector('i').className).to.includes('boss-icon-arrow-right');
    expect(getMenus(el).length).to.equal(2);
    getOptions(el, 1)[0].click();
    await wait(1000);
    getOptions(el, 2)[0].click();
