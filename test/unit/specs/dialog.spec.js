...
describe('Dialog', () => {
  ...
  it('create', done => {
    vm = createVue({
      template: `
        <div>
          <boss-dialog :title="title" :visible="visible"></boss-dialog>
        </div>
      `,
    ...
    setTimeout(() => {
      ...
      expect(vm.$el.querySelector('.boss-dialog__title').textContent).to.equal('dialog test');
  ...
  it('render correct content', done => {
    vm = createVue({
      template: `
        <div>
          <boss-dialog :title="title" :visible="visible">
            <span>这是一段信息</span>
    ...
    setTimeout(() => {
      const footerBtns = vm.$el.querySelectorAll('.boss-dialog__footer .boss-button');
      expect(vm.$el.querySelector('.boss-dialog__body span').textContent).to.equal('这是一段信息');
  ...
  it('append to body', done => {
    vm = createVue({
      template: `
        <div>
          <boss-dialog :title="title" append-to-body :visible="visible"></boss-dialog>
        </div>
      `,
  ...
  it('open and close', done => {
    vm = createVue({
      template: `
        <div>
          <boss-dialog :title="title" :visible.sync="visible">
            <span>这是一段信息</span>
          </boss-dialog>
        </div>
      `,
  ...
  describe('props', () => {
    const getDialogVm = (props, options) => {
      return createVue(Object.assign({
        template: `
          <div>
            <boss-dialog ${ props } :title="title" :visible="visible">
              <span>这是一段信息</span>
            </boss-dialog>
          </div>
        `,
    ...
    it('fullscreen', () => {
      ...
      const dialogEl = vm.$el.querySelector('.boss-dialog');
    ...
    it('top', () => {
      ...
      expect(vm.$el.querySelector('.boss-dialog').style.marginTop).to.equal('100px');
    });

    it('custom-class', () => {
      vm = getDialogVm('custom-class="my-dialog"');
      expect(vm.$el.querySelector('.boss-dialog').classList.contains('my-dialog')).to.true;
    });
  });
  ...
  it('click header btn', done => {
    ...
    const dialog = vm.$children[0];
    setTimeout(() => {
      dialog.$el.querySelector('.boss-dialog__headerbtn').click();
      setTimeout(() => {
        expect(vm.visible).to.be.false;
        done();
      }, 500);
  ...
  it('before close', done => {
    ...
    vm = createVue({
      template: `
        <div>
          <boss-dialog :title="title" :visible="visible" :before-close="beforeClose"></boss-dialog>
        </div>
      `,
  ...
  it('destroyOnClose', async() => {
    vm = createVue({
      template: `
        <div>
          <boss-dialog :title="title" :visible.sync="visible" destroy-on-close>
            <input />
          </boss-dialog>
        </div>
      `,
