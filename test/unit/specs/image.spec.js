...
describe('Image', () => {
  ...
  it('create', async() => {
    ...
    const placeholder = vm.$el.querySelector('.boss-image__placeholder');
    const error = vm.$el.querySelector('.boss-image__error');
    let img = vm.$el.querySelector('.boss-image__inner');
    expect(placeholder).to.exist;
    expect(error).to.be.null;
    expect(img).to.be.null;

    await wait();
    img = vm.$el.querySelector('.boss-image__inner');
  ...
  it('$attrs', async() => {
    ...
    await wait();
    const $img = vm.$el.querySelector('.boss-image__inner');
  ...
  it('pass event listeners', async() => {
    ...
    vm = createVue({
      template: `
        <boss-image @click="handleClick" :src="src"></boss-image>
      `,
      data() {
        return {
          src
    ...
    await wait();
    vm.$el.querySelector('.boss-image__inner').click();
    await wait();
  ...
  it('big image preview', async() => {
    vm = createVue({
      template: `
        <boss-image :src="src" :preview-src-list="srcList"></boss-image>
      `,
      data() {
        return {
          src: src,
    ...
    const $wrapper = vm.$el.querySelector('.boss-image-viewer__wrapper');
    expect($wrapper).to.exist;
    vm.$el.querySelector('.boss-image-viewer__close').click();
    await wait(1000);
