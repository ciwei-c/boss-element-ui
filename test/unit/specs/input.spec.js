...
describe('Input', () => {
  ...
  it('suffixIcon', () => {
    vm = createVue({
      template: `
        <boss-input suffix-icon="time"></boss-input>
      `
    }, true);
    var icon = vm.$el.querySelector('.boss-input__icon');
  ...
  it('prefixIcon', () => {
    vm = createVue({
      template: `
        <boss-input prefix-icon="time"></boss-input>
      `
    }, true);
    var icon = vm.$el.querySelector('.boss-input__icon');
    expect(icon).to.be.exist;
  ...
  it('size', () => {
    vm = createVue({
      template: `
        <boss-input size="large">
        </boss-input>
      `
    }, true);

    expect(vm.$el.classList.contains('boss-input--large')).to.true;
  });

  it('type', () => {
    vm = createVue({
      template: `
        <boss-input type="textarea">
        </boss-input>
      `
    }, true);

    expect(vm.$el.classList.contains('boss-textarea')).to.true;
  });

  it('rows', () => {
    vm = createVue({
      template: `
        <boss-input type="textarea" :rows="3">
        </boss-input>
      `
    }, true);
    expect(vm.$el.querySelector('.boss-textarea__inner').getAttribute('rows')).to.be.equal('3');
  });
  ...
  it('resize', async() => {
    vm = createVue({
      template: `
        <div>
          <boss-input type="textarea" :resize="resize"></boss-input>
        </div>
      `,
      data: {
        resize: 'none'
    ...
    await waitImmediate();
    expect(vm.$el.querySelector('.boss-textarea__inner').style.resize).to.be.equal(vm.resize);
  ...
  it('Input contains Select and append slot', async() => {
    vm = createVue({
      template: `
      <boss-input v-model="value" clearable class="input-with-select" ref="input">
    ...
    await wait();
    const suffixEl = document.querySelector('.input-with-select > .boss-input__suffix');
  ...
  it('validateEvent', async() => {
    ...
    vm = createVue({
      template: `
        <boss-form :model="model" :rules="rules">
          <boss-form-item prop="input">
            <boss-input v-model="model.input" :validate-event="false">
            </boss-input>
          </boss-form-item>
        </boss-form>
      `,
  ...
  describe('Input Events', () => {
    ...
    it('event:clear', async() => {
      ...
      await waitImmediate();
      vm.$el.querySelector('.boss-input__clear').click();
      await waitImmediate();
  ...
  it('sets value on textarea / input type change', async() => {
    vm = createVue({
      template: `
        <boss-input :type="type" v-model="val" />
      `,
      data() {
        return {
          type: 'text',
  ...
  it('limit input and show word count', async() => {
    ...
    expect(inputElm1.querySelectorAll('.boss-input__count').length).to.equal(0);
    expect(inputElm2.querySelectorAll('.boss-input__count').length).to.equal(1);
    expect(inputElm3.querySelectorAll('.boss-input__count').length).to.equal(0);
    expect(inputElm4.classList.contains('is-exceed')).to.true;

    vm.show = true;
    await waitImmediate();
    expect(inputElm1.querySelectorAll('.boss-input__count').length).to.equal(1);
