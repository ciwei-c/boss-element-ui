...
describe('Link', () => {
  ...
  it('create', () => {
    ...
    let linkElm = vm.$el;
    expect(linkElm.classList.contains('boss-link--primary')).to.be.true;
  });
  it('icon', () => {
    vm = createTest(Link, {
    ...
    let linkElm = vm.$el;
    expect(linkElm.querySelector('.boss-icon-search')).to.be.ok;
  });
  it('href', () => {
    vm = createTest(Link, {
  ...
  it('target', () => {
    vm = createVue(`
    <boss-link href="https://element.eleme.io" target="_blank">
      default
    </boss-link>
    `);
    let linkElm = vm.$el;
  ...
  it('click', async() => {
    ...
    vm = createVue({
      template: `
        <boss-link @click="handleClick"></boss-link>
      `,
      methods: {
        handleClick(evt) {
          result = evt;
