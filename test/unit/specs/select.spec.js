...
describe('Select', () => {
  ...
  it('create', () => {
    vm = createTest(Select, {value:''}, true);
    expect(vm.$el.className).to.equal('boss-select');
    expect(vm.$el.querySelector('.boss-input__inner').placeholder).to.equal('请选择');
    vm.toggleMenu();
  ...
  it('options rendered correctly', () => {
    vm = getSelectVm();
    const options = vm.$el.querySelectorAll('.boss-select-dropdown__item');
  ...
  it('custom dropdown class', () => {
    ...
    const dropdown = vm.$el.querySelector('.boss-select-dropdown');
  ...
  it('default value', done => {
    ...
    setTimeout(() => {
      expect(vm.$el.querySelector('.boss-input__inner').value).to.equal('双皮奶');
      done();
    }, 100);
  });
  ...
  it('single select', done => {
    vm = createVue({
      template: `
        <div>
          <boss-select v-model="value" @change="handleChange">
            <boss-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
  ...
  it('disabled select', () => {
    vm = createTest(Select, { disabled: true, value:'' }, true);
    expect(vm.$el.querySelector('.boss-input').classList.contains('is-disabled')).to.true;
  });
  ...
  it('visible event', done => {
    vm = createVue({
      template: `
        <div>
          <boss-select v-model="value" @visible-change="handleVisibleChange">
            <boss-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
  ...
  it('clearable', done => {
    ...
    setTimeout(() => {
      const iconClear = vm.$el.querySelector('.boss-input__icon.boss-icon-circle-close');
  ...
  it('object typed value', done => {
    vm = createVue({
      template: `
        <div>
          <boss-select v-model="value" value-key="id">
            <boss-option
              v-for="item in options"
              :label="item.label"
              :key="item.id"
    ...
    setTimeout(() => {
      expect(vm.$el.querySelector('.boss-input__inner').value).to.equal('label1');
      expect(vm.$el.querySelector('.boss-select-dropdown__item').classList.contains('selected'));
      done();
    }, 100);
  });
  ...
  it('prefixed icon', () => {
    ...
    expect(vm.$el.querySelector('.boss-input__icon').classList.contains('boss-icon-search')).to.be.true;
  });

  it('custom boss-option template', () => {
    vm = createVue({
      template: `
        <div>
          <boss-select v-model="value">
    ...
    expect(vm.$el.querySelector('.boss-select-dropdown__item p').textContent).to.equal('label value');
  });
  ...
  it('option group', () => {
    ...
    const groups = vm.$el.querySelectorAll('.boss-select-group__wrap');
    const options = groups[1].querySelectorAll('.boss-select-dropdown__item');
  ...
  it('allow create', done => {
    ...
    setTimeout(() => {
      ...
      select.onInputChange();
      setTimeout(() => {
        const options = document.querySelectorAll('.boss-select-dropdown__item span');
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          const tagCloseIcons = vm.$el.querySelectorAll('.boss-tag__close');
  ...
  it('multiple remove-tag', done => {
    ...
    vm = createVue({
      template: `
        <div>
          <boss-select v-model="value" multiple @remove-tag="handleRemoveTag">
            <boss-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
  ...
  it('event:focus & blur', done => {
    vm = createVue({
      template: `
        <boss-select ref="select" value=''></boss-select>
      `
    }, true);
  ...
  it('should return focus to input inside select after option select', done => {
    vm = createVue({
      template: `
        <div>
          <boss-select v-model="value" ref="select">
            <boss-option label="1" :value="1" />
          </boss-select>
        </div>
      `,
      data() {
    ...
    const option = vm.$el.querySelectorAll('.boss-select-dropdown__item')[0];
  ...
  it('only emit change on user input', done => {
    ...
    vm = createVue({
      template: `
        <div>
          <boss-select v-model="value" @change="change" ref="select">
            <boss-option label="1" :value="1" />
            <boss-option label="2" :value="2" />
            <boss-option label="3" :value="3" />
          </boss-select>
        </div>
      `,
      data() {
  ...
  it('should set placeholder to label of selected option when filterable is true and multiple is false', async() => {
    vm = createVue({
      template: `
        <div>
          <boss-select ref="select" v-model="value" filterable>
            <boss-option label="test" value="test" />
          </boss-select>
        </div>
      `,
      data() {
    ...
    expect(vm.$el.querySelector('.boss-input__inner').placeholder).to.be.equal('test');
  ...
  it('default value is null or undefined', async() => {
    ...
    vm.value = null;
    await waitImmediate();
    expect(vm.$el.querySelector('.boss-input__inner').value).to.equal('');
    vm.value = '选项1';
    await waitImmediate();
    expect(vm.$el.querySelector('.boss-input__inner').value).to.equal('黄金糕');
  });
