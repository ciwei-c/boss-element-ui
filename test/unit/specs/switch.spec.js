...
describe('Switch', () => {
  ...
  it('create', () => {
    ...
    const core = vm.$el.querySelector('.boss-switch__core');
    expect(core.style.backgroundColor).to.equal('rgb(255, 0, 0)');
    expect(core.style.width).to.equal('100px');
    expect(vm.$el.querySelector('.boss-switch__label--left').querySelector('span').textContent).to.equal('off');
  });

  it('switch with icons', () => {
    vm = createTest(Switch, {
      activeIconClass: 'boss-icon-check',
      inactiveIconClass: 'boss-icon-close'
    });

    const icon = vm.$el.querySelector('.boss-switch__label--left').querySelector('i');
  ...
  it('disabled switch should not respond to user click', done => {
    vm = createVue({
      template: `
        <div>
          <boss-switch disabled v-model="value"></boss-switch>
        </div>
      `,
    ...
    vm.$el.querySelector('.boss-switch__core').click();
    Vue.nextTick(() => {
  ...
  it('expand switch value', done => {
    vm = createVue({
      template: `
        <div>
          <boss-switch v-model="value" :active-value="onValue" :inactive-value="offValue"></boss-switch>
        </div>
      `,
      data() {
        return {
  ...
  it('value is the single source of truth', done => {
    vm = createVue({
      template: `
        <div>
          <boss-switch :value="true"></boss-switch>
        </div>
      `
    }, true);
