...
describe('Transfer', () => {
  ...
  const createTransfer = (props, opts) => {
    return createVue(Object.assign({
      template: `
        <boss-transfer :data="testData" ref="transfer" ${props}>
        </boss-transfer>
      `,
  ...
  it('filterable', done => {
    ...
    const leftList = transfer.$el.querySelector('.boss-transfer-panel').__vue__;
    leftList.query = '1';
    setTimeout(_ => {
  ...
  it('customize', () => {
    ...
    const label = transfer.querySelector('.boss-transfer-panel__header .boss-checkbox__label');
    expect(label.innerText.indexOf('表1') > -1).to.true;
    expect(transfer.querySelector('.boss-transfer-panel__list .boss-checkbox__label span').innerText).to.equal('1 - 备选项 1');
  ...
  it('check', () => {
    ...
    const leftList = vm.$refs.transfer.$el.querySelector('.boss-transfer-panel').__vue__;
  ...
  describe('target order', () => {
    it('original(default)', done => {
      ...
      setTimeout(() => {
        transfer.addToRight();
        setTimeout(() => {
          const targetItems = [].slice.call(vm.$el.querySelectorAll('.boss-transfer__buttons + .boss-transfer-panel .boss-transfer-panel__body .boss-checkbox__label span'));
